...
public class PyramideCYK {
	...
	private class Case{ // petite classe pour modéliser une case de la pyramide
		...
		public void addListProductions(ArrayList<String> list)
		{
			...
			{
				if(!productions.contains(list.get(i))) // Si la production n'y est pas déjà
	...
	public void addlistProductions(ArrayList<String> list, int numeroLigne, int numeroCase)
	{
		if(numeroLigne < lignesDeLaPyramide.size() && numeroCase < lignesDeLaPyramide.get(numeroLigne).length)
			lignesDeLaPyramide.get(numeroLigne)[numeroCase].addListProductions(list);
		else
			System.out.println("Erreur accès à une case incorrecte de la pyramide : ligne " + numeroLigne + " positionMot " + numeroCase);
	}
	
	public ArrayList<String> getListProductions(int numeroLigne, int numeroCase){
		
		if(numeroLigne < lignesDeLaPyramide.size() && numeroCase < lignesDeLaPyramide.get(numeroLigne).length)
			return lignesDeLaPyramide.get(numeroLigne)[numeroCase].getListProductions();
		else
			System.out.println("Erreur accès à une case incorrecte de la pyramide : ligne " + numeroLigne + " positionMot " + numeroCase);
		return lignesDeLaPyramide.get(numeroLigne)[numeroCase].getListProductions();
